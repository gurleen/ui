...
An npm-workspaces monorepo for the `@hydra-tv` React component libraries: `tokens` (CSS custom properties), `core` (generic components), `broadcast` (broadcast-TV control-room components, depends on `core`), plus a `playground` demo app. See the root `README.md` for the package map and quickstart commands.
...
`core` must build before `broadcast` (the root `build` script already runs them in that order) since `broadcast` imports `@hydra-tv/ui`'s built output, not its source, at type-check time.
