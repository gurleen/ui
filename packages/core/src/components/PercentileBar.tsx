      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <span style={{ position: "relative", flex: 1, minWidth: 40, height, background: "#0a0d10", borderRadius: "var(--radius-1)", boxShadow: "var(--inset-well)" }}>
          ...
          <span style={{
            ...
            fontFamily: "var(--font-data)", fontSize: 9, fontWeight: "var(--fw-bold)", fontFeatureSettings: "var(--numeric-features)", boxShadow: "var(--bevel-raised)",
          }}>{Math.round(pct)}</span>
        </span>
        {value !== undefined && (
          <span style={{ width: 60, flexShrink: 0, textAlign: "right", fontFamily: "var(--font-data)", fontSize: "var(--fs-11)", color: "var(--fg-1)", fontFeatureSettings: "var(--numeric-features)" }}>{value}</span>
        )}
      </div>
      {showScale && (
        <div style={{ display: "flex", justifyContent: "space-between", marginLeft: label && labelWidth > 0 ? labelWidth + 8 : 0, marginRight: value !== undefined ? 68 : 0, fontFamily: "var(--font-data)", fontSize: 9, color: "var(--fg-3)" }}>
