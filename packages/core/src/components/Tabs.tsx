...
export function Tabs({ tabs = [], active, defaultActive = 0, onChange, style }: TabsProps) {
  ...
  return (
    <div style={{ display: "flex", gap: 2, borderBottom: "1px solid var(--line-2)", fontFamily: "var(--font-mono)", ...style }}>
      {tabs.map((t, i) => {
        const on = i === idx;
        return (
          <button key={i} onClick={() => { if (active === undefined) setInternal(i); if (onChange) onChange(i); }}
            style={{
              ...
              fontFamily: "var(--font-mono)", fontWeight: on ? 600 : 400, cursor: "pointer",
