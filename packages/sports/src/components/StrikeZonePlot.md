...
| `showEmpty` | `boolean` | `false` | Keep the zone diagram at full size even when `pitches` is empty. Reserves the 4×5 ft aspect ratio in layout so the frame does not collapse. |
